...
/**
 * Написание текстов для описания коллекции.
 */
class CollectionTextTask extends Task
{
    ...
    public function run(Carbon $delay = null): void
    {
        ...
        foreach ($collections as $collection) {
            ...
            $entity->articleable_id = $collection->id;
            $entity->articleable_type = 'App\Modules\Collection\Models\Collection';

            $article = Article::create($entity->toArray());
            $job = ArticleWriteTextJob::dispatch($article->id, 'collection.text');
    ...
    private function getQuery(): Builder
    {
        return Collection::where('status', true)
            ->doesntHave('articles', 'and', function (Builder $query) {
                $query->where('articles.category', 'collection.text');
            })
            ->orderBy('id', 'ASC');
    }
}
