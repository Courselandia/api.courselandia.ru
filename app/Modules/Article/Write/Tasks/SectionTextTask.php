...
/**
 * Написание текстов для описания раздела.
 */
class SectionTextTask extends Task
{
    ...
    public function run(Carbon $delay = null): void
    {
        ...
        foreach ($sections as $section) {
            ...
            $entity->articleable_id = $section->id;
            $entity->articleable_type = 'App\Modules\Section\Models\Section';

            $article = Article::create($entity->toArray());
            $job = ArticleWriteTextJob::dispatch($article->id, 'section.text');
    ...
    private function getQuery(): Builder
    {
        return Section::where('status', true)
            ->doesntHave('articles', 'and', function (Builder $query) {
                $query->where('articles.category', 'section.text');
            })
            ->orderBy('id', 'ASC');
    }
}
