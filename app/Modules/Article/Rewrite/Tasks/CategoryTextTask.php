...
/**
 * Переписывание текстов для описания категорий.
 */
class CategoryTextTask extends Task
{
    ...
    public function run(Carbon $delay = null): void
    {
        ...
        foreach ($categories as $category) {
            ...
            $job = ArticleRewriteTextJob::dispatch($article->id, 'category.text', $article->text, $this->creative);
    ...
    private function getQuery(): Builder
    {
        ...
        ->with('articles', function ($query) {
            $query->where('category', 'category.text');
        })
