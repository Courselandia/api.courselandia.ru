...
/**
 * Переписывание текстов для описания инструмента.
 */
class ToolTextTask extends Task
{
    ...
    public function run(Carbon $delay = null): void
    {
        ...
        foreach ($tools as $tool) {
            ...
            $job = ArticleRewriteTextJob::dispatch($article->id, 'tool.text', $article->text, $this->creative);
    ...
    /**
     * Получить запрос на инструменты, у которых нет написанных статей для описания.
     *
     * @return Builder Построитель запроса.
     */
    private function getQuery(): Builder
    {
        return Tool::whereHas('articles', function ($query) {
        ...
        ->with('articles', function ($query) {
            $query->where('category', 'tool.text');
        })
